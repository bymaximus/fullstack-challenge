...
class ProdutoHistoricoTransformer extends Transformer
{
    /**
     * @var  array
     */
    ...
    public function transform(AuditModel $entity)
    {
        ...
        if ($valoresNovos &&
            Arr::has($valoresNovos, 'cor') &&
            $valoresNovos['cor']
        ) {
            $cores = json_decode($valoresNovos['cor'], true);
            $valoresNovos['cor'] = [];
            foreach ($cores as $cor) {
                if ($cor &&
                    ...
                    $valoresNovos['cor'][] = $cor['hex'];
                }
            }
        }
        if ($valoresAntigos &&
            Arr::has($valoresAntigos, 'cor') &&
            $valoresAntigos['cor']
        ) {
            $cores = json_decode($valoresAntigos['cor'], true);
            $valoresAntigos['cor'] = [];
            foreach ($cores as $cor) {
                if ($cor &&
                    Arr::has($cor, 'hex') &&
