	...
	<div class="vx-card p-6">
		...
		<vs-row class="mb-2">
			...
			>
				<v-select
					...
					v-model="provedorListaRegistros.state.searchFields['id_categoria']['value']"
		...
		<div class="bootstrap-inside">
			...
			>
				...
				<template v-slot:cell(categoria.nome)="row">
					<span class="text-nowrap">{{ row.value }}</span>
					<vx-tooltip
						color="danger"
						position="right"
						text="Categoria removida"
						v-if="(row.item && row.item.categoria && row.item.categoria.removido === true ? true : false)"
						:class="[
							{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
						]"
					>
				...
				<template v-slot:cell(subcategoria.nome)="row">
					<span class="text-nowrap">{{ row.value }}</span>
					<vx-tooltip
						color="danger"
						position="right"
						text="SubCategoria removida"
						v-if="(row.item && row.item.subcategoria && row.item.subcategoria.removido === true ? true : false)"
						:class="[
							{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
						]"
					>
...
export default {
	...
	watch: {
		"provedorListaRegistros.state.searchFields.id_categoria.value"(val) {
			...
			.then((response) => {
				if (
					...
					if (
						...
						this.listaCategoria = response.data.categorias;
					} else {
						this.$vs.notify({
							title: "Erro",
							text:
								"Incapaz de carregar a lista de categorias.",
							iconPack: "feather",
		...
		onListaRegistrosAtualizado() {
			this.listaRegistrosPaginaTotal =
				this.provedorListaRegistros.state.perPage > 0
					? this.provedorListaRegistros.totalRows >= this.provedorListaRegistros.state.perPage
						? Math.ceil(
